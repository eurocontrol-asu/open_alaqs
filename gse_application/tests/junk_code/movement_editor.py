...
class MovementEditor(QDialog):
    ...
    def refresh_summary_table(self):
        ...
        total_columns = (
            1 + len(self.gse_columns) + 3
        )  # code + gse_columns + [time, count, deterioration_factor]
        self.summary_table.setColumnCount(total_columns)

        headers = (
            ["Movement"] + self.gse_columns + ["time", "count", "deterioration_factor"]
        )
        ...
        for i, (code, assign) in enumerate(assigned):
            ...
            self.summary_table.setItem(
                i, 1 + len(self.gse_columns), QTableWidgetItem(str(assign.time))
            )
            self.summary_table.setItem(
                i, 2 + len(self.gse_columns), QTableWidgetItem(str(assign.count))
            )
            self.summary_table.setItem(
                i,
                3 + len(self.gse_columns),
                QTableWidgetItem(str(getattr(assign, "deterioration_factor", 1.0))),
            )
