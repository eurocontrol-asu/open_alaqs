...
class GSEDatabase:
    ...
    def _load_validated_csv(self, path: str, model: Any) -> List[Any]:
        ...
        if errors:
            raise RuntimeError(
                f"Validation errors in {os.path.basename(path)}:\n" + "\n".join(errors)
            )
        return records
    ...
    def _load_from_sqlite(self):
        ...
        try:
            ...
            self.emission_factors = [
                EmissionFactor.from_dict(row) for row in ef_df.to_dict("records")
            ]
            try:
                move_df = pd.read_sql("SELECT * FROM movements", conn)
                self.movements = [
                    Movement.from_dict(row) for row in move_df.to_dict("records")
                ]
            except Exception:
                self.movements = []
        except Exception as e:
    ...
    def _save_to_csv(self):
        db_dir = os.path.dirname(self.source)
        pd.DataFrame([asdict(x) for x in self.gse]).to_csv(
            os.path.join(db_dir, "default_gse.csv"), index=False
        )
        pd.DataFrame([asdict(x) for x in self.emission_factors]).to_csv(
            os.path.join(db_dir, "default_emission_factors.csv"), index=False
        )
        if self.movements:
            pd.DataFrame([asdict(x) for x in self.movements]).to_csv(
                os.path.join(db_dir, "movements.csv"), index=False
            )

    def _save_to_sqlite(self):
        conn = sqlite3.connect(self.source)
        try:
            pd.DataFrame([asdict(x) for x in self.gse]).to_sql(
                "gse", conn, if_exists="replace", index=False
            )
            pd.DataFrame([asdict(x) for x in self.emission_factors]).to_sql(
                "emission_factors", conn, if_exists="replace", index=False
            )
            if self.movements:
                pd.DataFrame([asdict(x) for x in self.movements]).to_sql(
                    "movements", conn, if_exists="replace", index=False
                )
        finally:
            conn.close()
