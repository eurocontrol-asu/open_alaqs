...
def dict_movement(movement_data):
    ...
    movement_dict['taxi_engine_count'] = float(movement_data[13]) if movement_data[13] else None
    movement_dict['set_time_of_main_engine_start_after_block_off_in_s'] = float(movement_data[14]) if movement_data[14] else None
    movement_dict['set_time_of_main_engine_start_before_takeoff_in_s'] = float(movement_data[15]) if movement_data[15] else None
    movement_dict['set_time_of_main_engine_off_after_runway_exit_in_s'] = float(movement_data[16]) if movement_data[16] else None
    movement_dict['engine_thrust_level_for_taxiing'] = float(movement_data[17]) if movement_data[17] else None
    movement_dict['taxi_fuel_ratio'] = float(movement_data[18]) if movement_data[18] else None
    movement_dict['number_of_stop_and_gos'] = float(movement_data[19]) if movement_data[19] else None
    movement_dict['domestic'] = str(movement_data[20])

    #for key_ in movement_dict:
    for key_ in ['runway_time', 'block_time', 'aircraft', 'gate', 'departure_arrival', 'runway']: #some fields are not mandatory
        if movement_dict[key_] is None:
            raise Exception("Field '%s' is '%s' or empty" % (key_, str(movement_dict[key_])))
...
def dict_engine(engine_data):
    ...
    engine_dict['pm10_nonvol'] = conversion.convertToFloat(engine_data[25],0.)
    engine_dict['pm10_sul'] = conversion.convertToFloat(engine_data[26],0.)
    engine_dict['pm10_organic'] = conversion.convertToFloat(engine_data[27],0.)
    engine_dict['nvpm_ei'] = conversion.convertToFloat(engine_data[28], 0.)
    engine_dict['nvpm_number_ei'] = \
        conversion.convertToFloat(engine_data[29], 0.)
