...
class Gate:
    def __init__(self, val=None):
        ...
        self._height = (
            ...
            if "gate_height" in val and not val["gate_height"] is None
            else 0.0
        )
    ...
    def getEmissionProfileGroups(self, source_type=""):
        ...
        for profile_ in self.getEmissionProfiles():
            if (
                source_type
                and not profile_.getEmissionSourceType().lower() == source_type.lower()
            ):
                continue
    ...
    def getEmissionProfile(self, ac_group, op_type, source_type):
        ...
        if not (ac_group is None or source_type is None):
            for profile_ in self.getEmissionProfiles():
                if (
                    profile_.getEmissionSourceType().lower() == source_type.lower()
                    and profile_.getAircraftGroup().lower() == ac_group.lower()
                    and profile_.getOPtype().lower() == op_type.lower()
                ):
                    found = profile_
        return found
    ...
    def initGate(self):
        for key, gate_dict in list(self.getGateDatabase().getEntries().items()):
            ...
            for _, profile in list(
                self.getDefaultGateEmissionProfiles().getObjects().items()
            ):
            ...
            self.setObject(
                gate_dict["gate_id"] if "gate_id" in gate_dict else "unknown", gate
            )
...
class DefaultGateEmissionProfile:
    def __init__(self, val=None):
        ...
        self._source_emission_type = (
            str(val["emis_type"]) if "emis_type" in val else None
        )
        self._source_emission_unit = (
            str(val["emis_unit"]).lower() if "emis_unit" in val else None
        )
        ...
        if not self.getEmissionSourceUnit() == "grams/hour":
            logger.error(
                "Wrong input field '%s' in default_gate_profiles: %s. Should be '%s'."
                % ("emis_unit", self.getEmissionSourceUnit(), "grams/hour")
            )
            raise Exception(
                "Wrong input field '%s' in default_gate_profiles: %s. Should be '%s'."
                % ("emis_unit", self.getEmissionSourceUnit(), "grams/hour")
        ...
        if not self.getTimeUnit() == "minutes":
            logger.error(
                "Wrong input field '%s' in default_gate_profiles: %s. Should be '%s'."
                % ("time_unit", self.getTimeUnit(), "minutes")
            )
            raise Exception(
                "Wrong input field '%s' in default_gate_profiles: %s. Should be '%s'."
                % ("time_unit", self.getTimeUnit(), "minutes")
...
class DefaultGateEmissionProfileStore(Store, metaclass=Singleton):
    ...
    def initDefaultGateEmissionProfile(self):
        for key, profile_dict in list(
            self.getDefaultGateEmissionProfileDatabase().getEntries().items()
        ):
            # add profile to store
            self.setObject(
                profile_dict["oid"] if "oid" in profile_dict else "unknown",
                DefaultGateEmissionProfile(profile_dict),
            )
