...
class SQLSerializable:
    ...
    def __init__(self, db_path_string, table_name_string,
        ...
        if not type(table_columns_type_dict)==type(self._table_columns):
            raise Exception("Expected type '%s' for columns-type mapping but got type '%s'." % (str(type(OrderedDict())), type(table_columns_type_dict)))
        else:
    ...
    def setEntry(self, key, value_object):
        if self.hasEntry(key):
            logger.warning("Already found entry with key '%s'. Replacing existing entry." %(str(key)))
    ...
    def serialize(self, path_=""):
        ...
        try:
            ...
            if result is not True:
                raise Exception("Error while creating table '%s'. Result was '%s'." % (self._table_name, str(result)))
            else:
                #insert rows
                result = self.insert_rows(path_, [self.getEntries()[key] for key in self.getEntries()])
                if result is not True:
                    raise Exception("Error while inserting rows to table '%s'. Result was '%s'." % (self._table_name, str(result)))
                else:
    ...
    def deserialize(self):
        ...
        if isinstance(result, str):
            ...
            logger.info("Table '%s' in database '%s' is empty."%(self._table_name, self._db_path))
        elif result is None:
            logger.error("Deserialization of database '%s' and table '%s' failed. Either file not found or table headers do not match expected headers." % (self._db_path, self._table_name))
        else:
            for row in result:
                values_dict = self.result_to_dict(columns_, row)
                if values_dict is None:
                    logger.error("Deserialization failed. Database '%s' with table '%s' returned dimension %i but expected %i." % (self._db_path, self._table_name, len(row), len(columns_)))
                else:
                    if not self.getPrimaryKey() in values_dict:
                        logger.error("Primary key '%s' is not contained in returned values. Database is '%s' with table '%s' and requested columns '%s'." % (self.getPrimaryKey(), self._db_path, self._table_name, str(columns_)))
                    else:
                        try:
        ...
        try:
            ...
            for index_, key_ in enumerate(self._table_columns.keys()):
                if not self._table_columns[key_]:
                    raise Exception("Could not create table. Column type for column '%s' is not valid." % (key_))
                if index_:
                    types_string += ","
            ...
            for geom_col_dict_ in self._geometry_columns:
                if all (k in geom_col_dict_ for k in ["column_name", "SRID", "geometry_type","geometry_type_dimension"]):
                    ...
                    if isinstance(result, str):
                        ...
                        logger.error("Table not created: Query 'AddGeometryColumn' returned False")
                        return False
