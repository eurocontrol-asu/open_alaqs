...
class QGISVectorLayerDispersionModule(OutputModule):
    ...
    def addDataPoint(self, grid, cell_hash, data_point):
        ...
        if not sum([abs(data_point["attributes"][attribute])>self.getThresholdToCreateDataPoint() for attribute in data_point["attributes"]]):
            logger.info("addDataPoint: Skip data point (%s) with value '%s' due to threshold (%s and of type '%s')."
                         %(str(data_point), str(sum([abs(data_point["attributes"][attribute])
                            for attribute in data_point["attributes"]])),str(self.getThresholdToCreateDataPoint()),
    ...
    def polygonize_bbox(self, box_):
        return Polygon([(box_['x_min'], box_['y_min']), (box_['x_max'], box_['y_min']),
                        (box_['x_max'], box_['y_max']), (box_['x_min'], box_['y_max'])])
    ...
    def getGridXYFromReferencePoint(self):
        ...
        try:
            ...
            user_set_factor = 1.0  # ToDo: Enlarge Calculation Grid by Factor set by the user
            self._x_left_border_grid = float(grid_origin_x) - user_set_factor*float(self._grid._x_resolution)
            self._y_left_border_grid = float(grid_origin_y) - user_set_factor*float(self._grid._y_resolution)

            return True
        except Exception as e:
            logger.error("ConcentrationsQGISVectorLayerOutputModule: Could not reset 3D grid origin from reference point: %s" % e)
            return False
    ...
    def assure_time_interval(self, output_data):
        try: #convert t1, t2 to dates and check timedelta
            t1, t2, t0 = output_data['t1'][0], output_data['t2'][0], output_data['rdat'][0]
            ...
            t2_hour = datetime.strptime(t2.split(".")[1],'%H:%M:%S').replace(year=start_date.year) if ("." in t2) \
            ...
            if self._averaging_period == "daily mean" :
                ...
                if (tdelta.total_seconds() == 86400.0):
                    ...
                    logger.debug("Error in timedelta for daily mean: (start: %s, end:%s, timedelta: %s)" % (t1, t2, tdelta))
                return False

            if self._averaging_period == "hourly" or self._averaging_period == "8-hours mean":
                #timedelta should be 1h
                tdelta = t2_hour - t1_hour
                if (tdelta.total_seconds() == 3600.0):
                    return True
                else:
                    logger.debug("Error in timedelta for hourly mean: (start: %s, end:%s, timedelta: %s)"%(start_time, end_time, tdelta))
                return False
    ...
    def readA2Koutput(self, datapath):
        if not os.path.isfile(datapath):
            QtWidgets.QMessageBox.warning(None, "File not found", "File '%s' not found. Choose another pollutant ? "%(datapath))
        try:
            with open(datapath, encoding="utf8", errors='ignore') as f:
                raw_output_data = [x.rstrip('\n').lstrip().replace('"','') for x in f.readlines()]
            ...
            concentration_matrix = np.loadtxt(raw_output_data, comments="*", skiprows=index_)
    ...
    def getA2KData(self):
        ...
        try:
            if self._averaging_period == "annual mean":
                output_file = os.path.join(self._concentration_database, '%s-y00a.dmna'%self._pollutant.lower()) if not self._check_uncertainty \
                    else os.path.join(self._concentration_database, '%s-y00s.dmna'%self._pollutant.lower())
                ...
                if (self._time_start < self._timeseries[0]) or (self._time_end > self._timeseries[-1]):
                    QtWidgets.QMessageBox.warning(None, "Error in time period",
                ...
                if self._averaging_period == "daily mean":
                    ...
                    for time_counter in range(dt_from_time_start.days, dt_from_time_end.days+1):
                        time_counter += +1

                        output_file = os.path.join(self._concentration_database, '%s-%sa.dmna'%(self._pollutant.lower(), str(time_counter).zfill(3))) if not self._check_uncertainty \
                            else os.path.join(self._concentration_database, '%s-%ss.dmna'%(self._pollutant.lower(), str(time_counter).zfill(3)))

                        output_data, index_, concentration_matrix = self.readA2Koutput(output_file)
                    ...
                    return output_data, index_, np.array(list(conc.values())).mean(axis=0)
                    #ToDo: Add max ?

                else:   # hourly (1h, 8h, whatever ..)
                    if self._averaging_period == "hourly":
                        for time_counter in range(int(dt_from_time_start.total_seconds()/3600), int(dt_from_time_end.total_seconds()/3600)):
                            time_counter += +1
                        ...
                        QtWidgets.QMessageBox.information(None, "8-hours mean",
                                                      "For time interval: %s (+8h)" %(self._time_start))

                        for time_counter in range(int(dt_from_time_start.total_seconds() / 3600),int(dt_from_time_start.total_seconds() / 3600)+8):
                            time_counter += +1
    ...
    def beginJob(self):
        try:
            ...
            self._data_cells = self._data_cells.assign(Q=pd.Series(0, index=self._data_cells.index))
            ...
            concentration_matrix_reshaped = np.reshape(concentration_matrix, (self._index_k, self._index_j, self._index_i))
            ...
            if self._check_uncertainty: # take only first level for uncertainty
                self._concentration_matrix = concentration_matrix_reshaped[0,:,:].squeeze()
            else:
            ...
            logger.error("ConcentrationsQGISVectorLayerOutputModule: Cannot complete beginJob: %s" % e)
            return False
    ...
    def process(self, **kwargs):
        ...
        for y, x in itertools.product(*list(map(range, (self._index_j, self._index_i)))):
            ...
            bbox = {'y_min': y0+(y)*self._delta, 'y_max': y0+(y)*self._delta,
                    'x_min': x0+(x)*self._delta, 'x_max': x0+(x)*self._delta,
                    'z_max': Zmax, 'z_min': 0}

            # ToDo: convert bbox to geom object ?
            geom = Point(x0+(x)*self._delta+self._delta*1./2, y0+(y)*self._delta+self._delta*1./2)
            # logger.info(geom)
            ...
            if conc_value >= self._threshold_to_create_a_data_point:
                ...
                matched_cells_2D = self._data_cells[self._data_cells.contains(geom) == True]
        ...
        if round(conc_value_counter,3) < round(self._total_concentration,3):
            logger.warning("A2K: Sum of all grid cells is < %s > rather than expected (%s)"
                        "The grid is most probably too small."%(conc_value_counter, self._total_concentration))
