...
class TimeSeriesDispersionModule(OutputModule):
    ...
    def assure_time_interval(self, output_data):
        try: #convert t1, t2 to dates and check timedelta
            ...
            if self._averaging_period == "daily mean" and (tdelta.total_seconds() == 86400.0):
                return True
            # if self._averaging_period == "1-h, 8-h, or else": timedelta should be 1h
            elif self._averaging_period != "daily mean" and (tdelta.total_seconds() == 3600.0):
                return True
            else:
                QtWidgets.QMessageBox.warning(None, "Time interval error", "Error in timedelta: (start: %s, end:%s)"%(t1, t2))
    ...
    def beginJob(self):
        ...
        if self._pollutant.startswith("PM"):
            logger.debug("Pollutant '%s', will be written simply as 'PM'"%self._pollutant)
            self._pollutant = "PM"
    ...
    def process(self):
        try:
            ...
            if self._averaging_period == "annual mean":
                QtWidgets.QMessageBox.information(None, "Error", "Cannot create time-series plot for annual mean")
                ...
                if self._averaging_period == "daily mean":
                    ...
                    self._data_x = [day_ for day_ in rrule.rrule(rrule.DAILY, dtstart=self._time_start, until=self._time_end + timedelta(days=-1))]
                    while time_counter <= (self._time_end - self._time_start).days:
                        ...
                        if not self.assure_time_interval(output_data):
                            QtWidgets.QMessageBox.warning(None, "Timedelta Error", "Make sure files do exist for averaging method: '%s'"%self._averaging_period)
                            logger.warning("Error in timedelta when reading dispersion model outputs. Choose another averaging period.")
                            return False
                        ...
                        if self._check_uncertainty: # take only first level for uncertainty: 0m to 3m
                            self._data_y.append(np.mean(concentration_matrix_reshaped[0,:,:].squeeze()))
                        else:
                    ...
                    self._data_x = [hour_ for hour_ in rrule.rrule(rrule.HOURLY, dtstart=self._time_start, until=self._time_end + timedelta(hours=-1))]
                    ...
                    if self._averaging_period == "8-hours mean":
                        if len(self._data_x) <= 8:
                            logger.warning("Not enough data to calculate 8h running mean")
                            return False
                        else:
                            # calculate 8h-moving mean
                            self._8h_runnning_average = self.CalculateMovingAverage(self._data_y, n=8)
                            self._8h_runnning_average_max = self.CalculateMovingAverage(self._data_y_max, n=8)
    ...
    def endJob(self):
        ...
        if self._data_x and self._data_y and self._data_y_std and len(self._data_x) == len(self._data_y):
            ...
            if not self._max_values:
                ...
                if self._averaging_period == "8-hours mean" and np.all(0 <= self._8h_runnning_average):
                    plot = self._widget.plot(self._data_x, self._data_y, self._options, lw=2, color='k')
                    leg = ['Daily mean']
                    no = 8
                    plot = self._widget.plot(self._data_x[no-1:], self._8h_runnning_average, self._options, lw=2, color='b', alpha=0.5)
                    leg.append('8h moving avg')

                else:
                    plot = self._widget.plot(self._data_x, self._data_y, self._options, lw=2, color='k')
                    leg = ['Daily mean'] if self._averaging_period == "daily mean" else ['Hourly mean']
                    y_plus_std = [self._data_y[i]+self._data_y_std[i] for i,j in enumerate(self._data_y)]
                    y_minus_std = [self._data_y[i]-self._data_y_std[i] for i,j in enumerate(self._data_y)]
                    axis.fill_between(self._data_x, y_plus_std, y_minus_std, lw=2, facecolor='k', alpha=0.5)

            else:
                plot = self._widget.plot(self._data_x, self._data_y_max, self._options, lw=2, color='k')
                leg = ['Daily max'] if self._averaging_period == "daily mean" else ['Hourly max.']
                self._title = (" ").join([leg[0], "concentration of %s"%self._pollutant])

                if self._averaging_period == "8-hours mean" and np.all(0 <= self._8h_runnning_average_max):
                    no = 8
                    plot = self._widget.plot(self._data_x[no-1:], self._8h_runnning_average_max, self._options, lw=2, color='b', alpha=0.5)
            ...
            legend = self._widget.getPlt().legend(leg, shadow=True, fancybox=True, loc = 'best')
            self._widget.getPlt().setp(legend.get_texts(),fontsize='medium') #sizes = ['xx-small', 'x-small', 'small', 'medium', 'large', 'x-large'
