...
class TimeSeriesWidgetOutputModule(OutputModule):
    ...
    def beginJob(self):
        ...
        try:
            ...
            for i in range(self._TableWidgetRows):
                ...
                if not self._receptor_df.dropna(how="any").empty and \
                        'longitude' in self._receptor_df.keys() and 'latitude' in self._receptor_df.keys():
                    ...
                    logger.info("Timeseries will be generated for the point: %s"%self._receptor_geom.to_wkt())
    ...
    def process(self, timeval, result, **kwargs):
        ...
        if self._receptor_df.dropna(how="any").empty:
            total_emissions_ = sum([_f for _f in [sum([_f for _f in emissions_ if _f]) for (source, emissions_) in result] if _f])

            if total_emissions_:
                self._data_x.append(timeval)

                if not self._pollutant is None:
                    self._data_y.append(total_emissions_.getValue(self._pollutant, unit="kg")[0])
                else:
                ...
                for em_ in emissions:
                    ...
                    try:
                        ...
                        isPoint_element_ = bool(
                            isinstance(em_.getGeometry(), Point))  # bool("POINT" in emissions_.getGeometryText())
                        isLine_element_ = bool(isinstance(em_.getGeometry(), LineString))
                        isMultiLine_element_ = bool(isinstance(em_.getGeometry(), MultiLineString))
                        ...
                        matched_cells_2D = self._griddata[self._griddata.intersects(geom) == True]
                        ...
                        if isLine_element_:
                            ...
                            matched_cells_2D.loc[matched_cells_2D.index, "Emission"] = EmissionValue / len(
                                matched_cells_2D)
                        ...
                        self._griddata.loc[matched_cells_2D.index, "Emission"] += matched_cells_2D["Emission"]
                    except Exception as exc_:
                        print(exc_)
                        continue

            receptor_cell = self._griddata[self._griddata.to_crs({'init': 'epsg:%s'%self._epsg}).intersects(self._receptor_geom)==True]
    ...
    def endJob(self):
        ...
        if self._data_x and self._data_y and len(self._data_x) == len(self._data_y):
            ...
            plot = self._widget.plot(self._data_x, self._data_y, self._marker)
