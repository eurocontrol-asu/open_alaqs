...
class TableViewDispersionModule(OutputModule):
    ...
    def process(self):
                ...
                if self._averaging_period == "daily mean":
                    ...
                    for pollutant_ in self._pollutant_list:
                        ...
                        while time_counter <= (self._time_end - self._time_start).days:
                            ...
                            output_file = os.path.join(self._concentration_database, '%s-%sa.dmna'%(pollutant_.lower(), str(time_counter).zfill(3))) if not self._check_uncertainty \
                                else os.path.join(self._concentration_database, '%s-%ss.dmna'%(pollutant_.lower(), str(time_counter).zfill(3)))
                    ...
                    for pollutant_ in self._pollutant_list:
                        ...
                        if self._averaging_period == "8-hours mean":
                            if len(self._data_x) <= 8:
                                ...
                                mov_average = self.CalculateMovingAverage(self._data_y_max, n=8) if self._max_values else self.CalculateMovingAverage(self._data_y, n=8)
        ...
        #write cells
        self._data_x = self._data_x[7:] if self._averaging_period == "8-hours mean" else self._data_x

        for index_row, time_interval in enumerate(self._data_x):
            time_ = time_interval.strftime('%Y-%m-%d') if self._averaging_period == "daily mean" else time_interval.strftime('%Y-%m-%d %H:%M')
    ...
    def endJob(self):
        ...
        if self._max_values:
            self._widget.setWindowTitle("Concentration results - %s (max. of all grid cells)"%self._averaging_period)
        else:
            self._widget.setWindowTitle("Concentration results - %s (mean of all grid cells)"%self._averaging_period)
