...
if __name__ == "__main__":
    ...
    dst_euro_standards_csv = Path(__file__).parents[1] / 'data/default_vehicle_fleet_euro_standards.csv'
    ...
    vehicle_categories = vehicle_categories.merge(euro_standards, how='left', on='euro_standard')
    ...
    for fleet_year in fleet_years:
        for _, country_vehicle_age in vehicle_age.iterrows():
            ...
            vc_available = vehicle_categories[vehicle_categories['introduction'] <= fleet_year]

            # Get the applicable euro standards for each vehicle category (corrected for average vehicle age)
            vc_applicable = vc_available[vc_available['introduction'] <= (fleet_year - age)]
