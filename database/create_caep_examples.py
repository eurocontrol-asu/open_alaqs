...
def sql_insert_into(table: str, data: dict) -> str:
    ...
    return (
        f"INSERT INTO {table} ({', '.join(column_names)}) VALUES ({', '.join(values)})"
    )
...
def create_caep_case_1():
    ...
    point_source_dict = {
        ...
        "source_co_kg_k": 0.0,
        "source_hc_kg_k": 0.0,
        "source_nox_kg_k": 31536.0,
        "source_sox_kg_k": 0.0,
        "source_pm10_kg_k": 0.0,
        "source_p1_kg_k": 0.0,
        "source_p2_kg_k": 0.0,
        "source_instudy": 1,
    ...
    meteorology_dict = {
        "Scenario": "default",
        "DateTime(YYYY-mm-dd hh:mm:ss)": datetime.utcnow().strftime(
            "%Y-01-01 00:00:00"
        ),
        "Temperature(K)": 288.5,
    ...
    try:
        ...
        con.commit()
        logging.info(
            "Added point source %s to database" % point_source_dict["source_id"]
        )
    except Exception as e:
        logging.error(e)
        logging.error(
            f'Couldn\'t add point source {point_source_dict["source_id"]} to database'
        )
    finally:
        con.close()
...
def create_caep_case_3_profile() -> pd.DataFrame:
    ...
    custom_aircraft_profile = pd.DataFrame({"tas_metres": tas, "horizontal_metres": s})
    ...
    custom_aircraft_profile["vertical_feet"] = (
        custom_aircraft_profile["vertical_metres"] / 0.3048
    )
    custom_aircraft_profile["horizontal_feet"] = (
        custom_aircraft_profile["horizontal_metres"] / 0.3048
    )
...
def create_caep_case_3():
    ...
    try:
        ...
        logging.error(
            f"Could not add taxi route {taxiroute_dict['route_name']} to database"
        )
    finally:
        con.close()
    ...
    runway_times = runway_date + pd.Series(runway_time_deltas) * pd.to_timedelta(
        1, unit="h"
    )
