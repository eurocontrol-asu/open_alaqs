...
if __name__ == "__main__":
    ...
    example_path = EXAMPLES_DIR / 'forum_out.alaqs'
    ...
    shutil.copy(original_database, example_path)
    ...
    apply_sql(example_engine, sql_files, file_type='inventory')
    ...
    for csv_path in csv_files:
        ...
        project_data = pd.read_sql(f"SELECT * FROM {csv_path.stem}", example_engine)
        ...
        if data is not None and project_data.empty and not data.empty:
            try:
                data.to_sql(csv_path.stem, example_engine, index=False, if_exists='append')
