...
def form_open(form, layer, feature):
    ...
    fields['type'].currentTextChanged.connect(
        lambda v: change_type_field(fields, v))
    ...
    def on_save():
        ...
        form.changeAttribute("hour_profile",
                             fields['hour_profile'].currentText())
...
def change_category_field(fields: dict, category: str):
    ...
    if (category_data is None) or (category_data == []):
        raise Exception(
            "The selected category returned no data: %s" % category_data)
    ...
    if isinstance(category_types, str):
        raise Exception(
            "Category types could not be returned: %s" % category_types)
...
def change_type_field(fields: dict, type_name: str):
    ...
    if isinstance(type_data, str):
        raise Exception(
            "Could not return category type data: %s" % type_data)
    ...
    if substance == 1:
        ...
        raise Exception(
            "Substance could not be identified for this point type.")
