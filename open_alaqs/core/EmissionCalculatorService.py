...
class EmissionCalculatorService:
    ...
    def validate_config(self, config: EmissionCalculationConfig) -> tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            if config.pollutant not in self.supported_pollutants:
                return False, f"Invalid pollutant: {config.pollutant}. Supported: {self.supported_pollutants}"
            
            if config.method not in self.supported_methods:
                return False, f"Invalid method: {config.method}. Supported: {self.supported_methods}"
            ...
            if config.should_apply_nox_corrections and config.method == "BFFM2":
                self._warnings.append("NOx corrections are not applicable with BFFM2 method")
    ...
    def calculate_emissions(self, config: EmissionCalculationConfig) -> EmissionCalculationResult:
        """
    ...
    def _log_configuration(self, config: EmissionCalculationConfig) -> None:
        ...
        has_receptors = config.receptor_points is not None and len(config.receptor_points) > 0
        logger.info(f"Has Receptor Points: {has_receptors}")
    
    def _initialize_calculation(self, config: EmissionCalculationConfig) -> EmissionCalculation:
