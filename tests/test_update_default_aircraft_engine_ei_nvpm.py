...
def test_calculate_nvpm_mass_and_number_tx(sample_data: pd.DataFrame):
    ...
    assert (
        apply_lower_limit(theoretical_values[0])
        <= ei_nvpm_mass_ek
        <= apply_upper_limit(theoretical_values[0])
    )
    assert (
        apply_lower_limit(theoretical_values[1])
        <= ei_nvpm_number_ek
        <= apply_upper_limit(theoretical_values[1])
    )
