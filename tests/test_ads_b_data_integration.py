...
def test_ads_b_validation(plugin_instance):
    ...
    db_path = str(get_copy_path(get_data_path("AIRPORT_A") / "AIRPORT_A_out.alaqs"))
    ...
    ads_b_file_invalid = str(
        get_copy_path(
            get_data_path("AIRPORT_A/ADS-B") / "AIRPORT_A_ads_b_data_invalid_2.csv"
        )
    )
    res, msg = validate_adsb_file(ads_b_file_invalid)
    assert not res
    ...
    ads_b_file_invalid = str(
        get_copy_path(
            get_data_path("AIRPORT_A/ADS-B") / "AIRPORT_A_ads_b_data_invalid_3.csv"
        )
    )
    res, msg = validate_adsb_file(ads_b_file_invalid)
    assert not res
    ...
    ads_b_file_valid = str(
        get_copy_path(
            get_data_path("AIRPORT_A/ADS-B") / "AIRPORT_A_ads_b_data_valid.csv"
        )
    )
