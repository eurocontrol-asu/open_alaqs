...
def test_segment_fully_inside_grid(grid_bounds):
    ...
    clip_x1, clip_y1, clip_z1, clip_x2, clip_y2, clip_z2, fraction = spatial.clip_segment_to_grid(
...
def test_linestring_crossing_grid_completely(grid_bounds):
    ...
    first_x, first_y, first_z = float(coords[0][0]), float(coords[0][1]), float(coords[0][2])
    ...
    last_x, last_y, last_z = float(coords[-1][0]), float(coords[-1][1]), float(coords[-1][2])
...
def test_trajectory_segment_preserves_all_metadata(grid_bounds):
    ...
    start_data = start._data if hasattr(start, '_data') else {}
    end_data = end._data if hasattr(end, '_data') else {}
...
def test_consistency_between_methods_single_segment(grid_bounds):
    ...
    clip_x1_core, clip_y1_core, clip_z1_core, clip_x2_core, clip_y2_core, clip_z2_core, frac_core = \
...
def test_consistency_between_methods_linestring(grid_bounds):
    ...
    clip_x1, clip_y1, clip_z1, clip_x2, clip_y2, clip_z2, frac = spatial.clip_segment_to_grid(
        x1, y1, z1, x2, y2, z2, grid_bounds
    )
