...
def aggregate_emissions(emissions_data: dict) -> dict:
    ...
    totals = {
        "fuel_kg": 0.0, "co_kg": 0.0, "co2_kg": 0.0, "hc_kg": 0.0,
        "nox_kg": 0.0, "sox_kg": 0.0, "pm10_kg": 0.0, "p1_kg": 0.0,
        "p2_kg": 0.0, "pm10_organic_kg": 0.0, "pm10_prefoa3_kg": 0.0,
        "pm10_nonvol_kg": 0.0, "pm10_sul_kg": 0.0,
    }
...
def load_expected_totals_from_csv(csv_path: str) -> dict:
    ...
    pollutant_columns = [
        ...
        "p1_kg", "p2_kg", "pm10_organic_kg", "pm10_prefoa3_kg",
        "pm10_nonvol_kg", "pm10_sul_kg"
    ]
    return {col: df[col].sum() if col in df.columns else 0.0 for col in pollutant_columns}
...
def load_expected_from_csv_single_row(csv_path: str, row_index: int = 0) -> dict:
    ...
    return {col: df.iloc[row_index][col] if col in df.columns else 0.0 for col in pollutant_columns}


def compare_emissions_with_expected(calculated, expected, rel_tol=1e-6, pollutants=None):
    ...
    for pollutant in pollutants:
        ...
        assert calc_val == pytest.approx(exp_val, rel=rel_tol), (
